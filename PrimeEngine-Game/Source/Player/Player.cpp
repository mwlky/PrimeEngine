...
{
    Player::Player(PlayerAnimationPack animation_pack, Prime::Vector2 startPosition)
    {
        Prime::Entity entity;

        m_Entity = entity;
        m_Transform = m_Entity.AddComponent<Prime::Transform>();
        m_Sprite = m_Entity.AddComponent<Prime::Sprite>(animation_pack.WalkDownTextures[0]);
    ...
    void Player::TickPlayer()
    {
        ...
        {
            m_Transform->Velocity = Prime::Vector2().Up() * -1 * m_Speed;
            PlayAnimation(m_Animations.WalkUpTextures, 200);
        }
        else if (keyStates[SDL_SCANCODE_A])
        {
            m_Transform->Velocity = Prime::Vector2().Left() * m_Speed;
            PlayAnimation(m_Animations.WalkLeftTextures, 110);
        }
        else if (keyStates[SDL_SCANCODE_S])
        {
            m_Transform->Velocity = Prime::Vector2().Down() * -1 * m_Speed;
            PlayAnimation(m_Animations.WalkDownTextures, 200);
        }
        else if (keyStates[SDL_SCANCODE_D])
        {
            m_Transform->Velocity = Prime::Vector2().Right() * m_Speed;
            PlayAnimation(m_Animations.WalkRightTextures, 110);
        }
        else
        {
            m_Transform->Velocity = Prime::Vector2().Zero();
        }
    }
    ...
    void Player::PlayAnimation(std::vector<SDL_Texture*> animations, float delay)
    {
        ...
        m_Entity.GetComponent<Prime::Sprite>()->ChangeSprite(animations[m_CurrentAnimationIndex]);
    }
} // Prime
